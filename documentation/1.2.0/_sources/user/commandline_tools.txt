...
Hadoop ships with an implementation of an S3 Native filesystem denoted by the prefix ``s3n://`` that can be enabled in
the Hadoop configuration used with GeoMesa Tools. Edit the core-site.xml file in your Hadoop installation. These
instructions apply to Hadoop 2.5.0 and higher. Note that you must have the environmental variable
``HADOOP_MAPRED_HOME`` set properly in your environment.
...
After you have enabled S3 in your Hadoop configuration you can ingest with GeoMesa tools. Note that you can still use
the Kleene star (*) with S3. S3 paths are prefixed in hadoop with ``s3n://`` as shown below::
