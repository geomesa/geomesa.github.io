...
If there are no ``AuthorizationsProvider``s found on the classpath, and the ``auths`` parameter is not set, GeoMesa will default to
...
The rest of this tutorial will use the GDELT data set, described in the [GDELT Map-Reduce tutorial](/geomesa-gdelt-analysis/). If you have never ingested GDELT data, or you have previously ingested it **without** visibilities, you will need to ingest it again.
...
Basic user authentication will take place via user certificates. Each user will have their own public/private
...
<li>
If you want to reuse your existing GeoServer configuration, create an environment variable pointing to your GeoServer data directory
(you may want to add this to your shell initialization scripts):
{% highlight bash %}
...
<li>
Start Tomcat, either as a service or through the startup scripts, and ensure that GeoServer is available at <a href='http://localhost:8080/geoserver/web/'>http://localhost:8080/geoserver/web/</a>.
</li>
...
*Note: There is a bug in some versions of GeoServer, where it sometimes does not save
...
<li>Navigate to the GeoServer data directory (<code>$GEOSERVER_DATA_DIR</code> or <code>$GEOSERVER_HOME/data_dir</code>)</li>
<li>Edit the file <code>./security/config.xml</code> by adding the 4 lines below:
...
The tutorial code includes an ``AuthorizationsProvider`` implementation that will connect to LDAP to retrieve authorizations, in the class ``geomesa.tutorial.LdapAuthorizationsProvider``.
...
The tutorial code includes a test case for connecting to LDAP, in the class ``geomesa.tutorial.LdapAuthorizationsProviderTest``.
...
<ol>
<li>Ensure that your LDAP configuration is correct by running ``LdapAuthorizationsProviderTest``, as
described above.</li>
<li>Change the provider class in <code>src/main/resources/META-INF/services/geomesa.core.security.AuthorizationsProvider</code>
to be <code>geomesa.tutorial.LdapAuthorizationsProvider</code></li>
<li>Rebuild the tutorial JAR and install the <b>unshaded original</b> jar in GeoServer:
...
* ```<url>```: the **_HTTPS_** path to GeoServer, e.g. ``https://localhost:8443/geoserver/``
...
**_Note: Ensure that the URL for GeoServer is using HTTPS.*
...
The code for querying through WFS is available in the class ```geomesa.tutorial.GeoServerAuthorizationsTutorial```. The interesting code for this tutorial is contained in the ``main`` method:
